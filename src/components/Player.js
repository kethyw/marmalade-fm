...
class Player extends Component {
  ...
  mountAudio = async () => {
    ...
    // here we want our widget to be ready before continuing
    await this.widget.ready;

    // here we set out widget state to be ready in redux so 
    // we can block anything from happening befire it's ready 
    setWidgetReady(true);
  ...
  componentDidMount() {
    ...
    // everything is ready, so we then run our amountAudio()
    // method 
    this.mountAudio();
  };
  ...
  render() {
    return (
      <iframe
        width="100%"
        height="60"
        src="https://www.mixcloud.com/widget/iframe/?hide_cover=1&mini=1&feed=%2Fclassical-music%2Fclassical-music-the-essential-collection-vol1%2F"
        frameBorder="0"
